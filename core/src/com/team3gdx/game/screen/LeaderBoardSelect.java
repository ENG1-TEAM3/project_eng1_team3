...
public class LeaderBoardSelect implements Screen{
	 ...
	 */
	public LeaderBoardSelect(MainGameClass game, MainScreen ms) {
		this.game = game;
		this.ms = ms;
	...
	public void show() {
		...
		endless.addListener(new ClickListener() {
			public void touchUp(InputEvent event, float x, float y, int pointer, int button) {
				state = STATE.leaderboard;
				gameMode = new EndlessMode(3,60000);
				super.touchUp(event, x, y, pointer, button);
			}
		});
	...
	public void changeScreen(STATE state) {
		if (state == STATE.leaderboard) {
			...
			game.setScreen(new LeaderBoard(game, ms, gameMode));
		}
		if (state == STATE.main) {
			game.mainScreenMusic.dispose();
