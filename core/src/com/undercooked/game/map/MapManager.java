...
public class MapManager {
    ...
    public Map load(String path, StationController stationController, CookController cookController, Interactions interactions, Items gameItems) {
        ...
        if (root == null) {
            ...
            if (path != Constants.DEFAULT_MAP){
                return load(Constants.DEFAULT_MAP, stationController, cookController, interactions, gameItems);
            } else {
                return null;
            }
        }
    ...
    public static Map mapOfSize(int width, int height, StationController stationController, TextureManager textureManager, AudioManager audioManager, Interactions interactions, Items gameItems) {
        ...
        for (int i = 0 ; i < width ; i++) {
            returnMap.addMapEntity(newCounter(stationController, audioManager, interactions, gameItems),i,0,null);
            returnMap.addMapEntity(newCounter(stationController, audioManager, interactions, gameItems),i,height-1,null);
        }
        // Y entities
        for (int j = 1 ; j < height-1 ; j++) {
            returnMap.addMapEntity(newCounter(stationController, audioManager, interactions, gameItems),0,j,null);
            returnMap.addMapEntity(newCounter(stationController, audioManager, interactions, gameItems),width-1,j,null);
        }

        // Leftmost wall
        for (int j = 1 ; j < fullHeight ; j++) {
            returnMap.addFullMapEntity(newCounter(stationController, audioManager, interactions, gameItems),0,j,null);
        }

        // Add 2 at the top at x=1, x=2
        returnMap.addFullMapEntity(newCounter(stationController, audioManager, interactions, gameItems), 1, fullHeight-1,null);
        returnMap.addFullMapEntity(newCounter(stationController, audioManager, interactions, gameItems), 2, fullHeight-1,null);
